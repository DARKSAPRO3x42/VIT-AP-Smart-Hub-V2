...
class AndroidEnvironment {
  ...
  Future<Map<String, String>> buildEnvironment() async {
    ...
    final arKey = 'AR_${target.rust.replaceAll('-', '_')}';
    ...
    final ccKey = 'CC_${target.rust.replaceAll('-', '_')}';
    final ccValue = path.join(toolchainPath, 'clang$exe');
    final cfFlagsKey = 'CFLAGS_${target.rust.replaceAll('-', '_')}';
    final cFlagsValue = targetArg;

    final cxxKey = 'CXX_${target.rust.replaceAll('-', '_')}';
    final cxxValue = path.join(toolchainPath, 'clang++$exe');
    final cxxFlagsKey = 'CXXFLAGS_${target.rust.replaceAll('-', '_')}';
    final cxxFlagsValue = targetArg;

    final linkerKey =
        'cargo_target_${target.rust.replaceAll('-', '_')}_linker'.toUpperCase();

    final ranlibKey = 'RANLIB_${target.rust.replaceAll('-', '_')}';
